...
export function showBrandDetail(brand, tasks, contracts, { onClose }) {
  const sheet = document.getElementById('detailSheet'), overlay = document.getElementById('detailOverlay');
  if (!sheet || !overlay) return;
  const daysToEnd = getDaysUntil(brand.leaseEnd), leaseStatus = daysToEnd <= 30 && daysToEnd >= 0 ? '<span class="tag tag-warning">⚠ ' + daysToEnd + '天后到期</span>' : daysToEnd < 0 ? '<span class="tag tag-danger">已过期</span>' : '';
  const relatedTasks = tasks.filter(t => (t.brandName || t.品牌) === brand.name);
  sheet.innerHTML = `<h2>🏢 ${escHtml(brand.name)}</h2><div style="background:var(--bg);border-radius:var(--radius-sm);padding:14px;margin-bottom:12px"><div style="display:flex;flex-wrap:wrap;gap:8px;font-size:13px;color:var(--sub);margin-bottom:8px">${brand.floor ? '<span>📍 ' + escHtml(brand.floor) + '楼</span>' : ''}${brand.category ? '<span>🏷 ' + escHtml(brand.category) + '</span>' : ''}${brand.area ? '<span>📐 ' + brand.area + '㎡</span>' : ''}${brand.monthlyRent ? '<span>💰 ¥' + brand.monthlyRent + '/月</span>' : ''}${leaseStatus}</div>${brand.leaseStart || brand.leaseEnd ? '<div style="font-size:12px;color:var(--sub)">📅 租期：' + (brand.leaseStart || '?') + ' ~ ' + (brand.leaseEnd || '?') + '</div>' : ''}${brand.contacts && brand.contacts.length ? '<div style="margin-top:8px"><div style="font-size:12px;color:var(--sub);margin-bottom:4px">👥 联系人</div>' + brand.contacts.map(c => '<div style="font-size:13px;margin-bottom:2px">' + escHtml(c.name) + (c.role ? ' · ' + escHtml(c.role) : '') + (c.phone ? ' · ' + escHtml(c.phone) : '') + '</div>').join('') + '</div>' : ''}${brand.notes ? '<div style="margin-top:8px;font-size:13px;color:var(--sub)">📝 ' + escHtml(brand.notes) + '</div>' : ''}</div>${relatedTasks.length ? '<div style="font-size:13px;font-weight:600;color:var(--sub);margin-bottom:8px">📋 关联任务 (' + relatedTasks.length + ')</div>' + relatedTasks.slice(0, 5).map(t => '<div style="font-size:13px;padding:6px 0;border-bottom:1px solid var(--border)">' + ((t.status || t.状态) === '已完成' ? '✅ ' : '○ ') + escHtml(t.content || t.任务内容 || '') + '</div>').join('') : ''}<div class="modal-actions"><button class="btn-cancel" id="brandDetailClose">关闭</button></div>`;
